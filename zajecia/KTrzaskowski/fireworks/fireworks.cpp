...
//    al_clear_to_color(al_map_rgb(0,0,0));
    ...
    for (int i = 0; i < n; i++) {
        ...
        al_draw_filled_circle(o[i].x, o[i].y, 4, o[i].col);
    }
}
...
void aktualizuj_plansze()
{
    for (int i = 0; i < n; i++) {
		...
		o[n].col = al_map_rgb(random() % 256, random() % 256,random() % 256);                
                n++;
            }
        }
...
void inicjalizacja()
{
    ...
    o[n].x = screen_w / 2;
    o[n].y = screen_h - 20;
    o[n].dx = (float) (random() % 250) / 50.0 - 2.5;
    o[n].dy = -2.2 - (float) (random() % 100) / 100.0;
    o[n].size = 4;
    o[n].life = default_life;
    o[n].col = al_map_rgb(random() % 256, random() % 256,random() % 256);
    n++;
